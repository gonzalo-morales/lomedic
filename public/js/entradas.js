...
$('#fk_id_tipo_documento').on('change', function() {
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            $('#entrada_escaner').empty();
            $.each(data, function(index,value) {
                // alert(index +' '+ value);
                $('#entrada_escaner').append('<option value="'+ value +'">'+ value +'</option>');
            });
            $('#entrada_escaner').val('');
...
$('#entrada_escaner').on('change', function() {
    ...
    $.ajax({
        ...
        data: {'fk_id_tipo_documento':$('#fk_id_tipo_documento').val(),
            '_token':token,
            'entrada_escaner':$('#entrada_escaner').val(),
        },
        dataType: "json",
        success:function(data){
            console.info(data);
            $.each(data.detalle, function(index,value) {
                ...
                $('#table_detalle').append('<tr>' +
                    '<td class="codigo">'+ value.sku +'</td>' +
                    '<td class="codigo">'+ value.upc +'</td>' +
                    // '<td class="'+ value.sku +'">'+ value.sku +'</td>' +
                    // '<td class="'+ value.upc +'">'+ value.upc +'</td>' +
...
// $(document).on('change','#codigo_producto',function (el)
$('#codigo_producto').on('change', function()
{
    var codigo = $('#codigo_producto').val();

    // console.info($('.codigo').parent().find(codigo).html());
    // console.info($('.codigo').parent());
    //console.info($('.codigo').parent());
    ...
    $.each($('.codigo'), function(index,value) {
      ...
      var datos = $(value).parent().find('td');

      if( $(datos[0]).html() == codigo || $(datos[1]).html() == codigo)
      {
          if( (parseInt($(datos[8]).html())+parseInt($(datos[9]).html())+1) <= parseInt($(datos[7]).html())  )
          {
              $(datos[9]).html(parseInt($(datos[9]).html())+1);
              $('#codigo_producto').val('');
              return false;
          }
      ...
      // console.info($(value).parent().find('td'));
