...
<div class="container-fluid">
	<div id="metro" class="row col-sm-12 mb-3" style="opacity: 0; top:50px;">
		@if(isset($menu) && !empty($menu->where('id_modulo',8)))
			@foreach($menu->where('id_modulo',8) as $key=>$values)
				@each('partials.wmenu', $menu->where('id_modulo',8)[$key]->submodulos , 'modulo')
			@endforeach
		@endif
	</div>
</div>
