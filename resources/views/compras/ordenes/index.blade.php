...
<div class="row">
    <div class="col s12">
        <section id="smart-view" class="row" data-primary-key="{{ currentEntity()->getKeyName() }}" data-columns="{{ json_encode(array_keys($fields)) }}" data-item-show-or-delete-url="{{ companyRoute('show', ['id' => '#ID#']) }}" data-item-update-url="{{ companyRoute('edit', ['id' => '#ID#']) }}">
            ...
            <div class="col s12 m12">
                ...
                <div class="row" rv-show="actions.countItems | call < collections.items" style="display: none;">
                    <div class="right">
                        ...
                        <button class="btn waves-effect waves-light" rv-on-click="actions.showModalMotivoCancelacion" data-delete-type="multiple" data-delete-url="{{companyRoute('destroyMultiple')}}"><i class="material-icons left">delete</i>Eliminar (<span rv-text="actions.countItems | call < collections.items"></span>)</button>
                        @endcan
            ...
            <div class="col s12 ml2">
                <table class="smart-table striped responsive-table highlight">
                    ...
                    <tr>
                        ...
                        @endforeach
                        <td class="width-auto">
                            <span rv-get-item-id-and-estatus data-item-id="{{$row->getKey()}}" data-item-estatus="{{$row->estatus->id_estatus}}"></span>
                        </td>
                    </tr>
                    @endforeach
                    </tbody>
            ...
            <div id="modal-delete" class="modal">
                <div class="modal-content">
                    <h5>¿Cuál es el motivo de la cancelación?</h5>
                    <p>Una vez cancelado no podrás recuperarlo.</p>
                    {!! Form::textarea('motivo_cancelacion',null,['id'=>'motivo_cancelacion','class'=>'materialize-textarea']) !!}
                </div>
                <div class="modal-footer">
                    <button class="modal-action modal-close waves-effect waves-light btn orange" rv-on-click="actions.itemsCancelacion">Aceptar</button>
        ...
        <div class="smart-actions" hidden>
            ...
            @can('delete', currentEntity())
            @endcan
            <a href="#" class="waves-effect waves-light btn btn-flat no-padding" rv-on-click="actions.showModalMotivoCancelacion" rv-get-datarow rv-get-delete-url data-item-id="#ID#" data-item-estatus="#ESTATUS#" rv-hide-delete data-delete-type="single" ><i class="material-icons">delete</i></a>
        </div>
    </div>
</div>
@endsection