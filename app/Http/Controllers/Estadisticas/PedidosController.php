...
class PedidosController extends ControllerBase
{
	...
	{
	    return view('estadisticas.pedidos.index',[
	...
	public function store(Request $request, $company, $compact = false)
	{
	    ...
	    $char1 = DB::table('inv_pedido as p')
    	    ->leftJoin('inv_pedido_detalle as d','d.id_pedido','p.id_pedido')
    	    ...
    	    ->whereBetween(DB::RAW("to_char(p.fecha_pedido, 'YYYY-MM-DD')"), [$fecha_inicio, $fecha_fin])
	    ...
	    $char01 = DB::table('inv_pedido as p')
    	    ->leftJoin('inv_pedido_detalle as d','d.id_pedido','p.id_pedido')
    	    ->leftJoin('cat_localidad as l', function($q) use($localidad){
    	        $q->on('l.id_localidad','=','p.id_localidad');
    	        $q->whereRaw("(l.id_localidad = $localidad or $localidad = -999)");
	    ...
	    $char2 = DB::table('inv_pedido as p')
    	    ->leftJoin('inv_pedido_detalle as d','d.id_pedido','p.id_pedido')
    	    ->leftJoin('cat_localidad as l', function($q) use($localidad){
    	        $q->on('l.id_localidad','=','p.id_localidad');
    	        $q->whereRaw("(l.id_localidad = $localidad or $localidad = -999)");
	    ...
	    $char3 = DB::table('inv_pedido as p')
    	    ->leftJoin('inv_pedido_detalle as d','d.id_pedido','p.id_pedido')
    	    ->leftJoin('cat_localidad as l', function($q) use($localidad){
    	        $q->on('l.id_localidad','=','p.id_localidad');
    	        $q->whereRaw("(l.id_localidad = $localidad or $localidad = -999)");
