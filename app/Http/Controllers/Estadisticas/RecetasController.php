...
class RecetasController extends ControllerBase
{
	public function index($company, $attributes = ['where'=>[]])
	{
	    return view('estadisticas.recetas.index',[
	        'jurisdicciones'=>Jurisdicciones::whereHas('sucursales', function($q){ $q->activos(); })->pluck('jurisdiccion','id_jurisdiccion')->prepend('TODAS LAS JURISDICCIONES','-999'),
	        'localidades' => Sucursales::activos()->where('tipo',0)->pluck('sucursal','id_sucursal')->prepend('TODOS LOS CENTROS DE SALUD','-999'),
	    ]);
	}
	...
	public function store(Request $request, $company, $compact = false)
	{
	    ...
	    $char1 = DB::table('rec_opr_receta as p')
    	    ->leftJoin('rec_det_receta as d','d.id_receta','p.id_receta')
	    ...
	    $char01 = DB::table('ss_qro_receta as p')
        ...
        $char02 = DB::table('ss_qro_receta as p')
            ->leftJoin('ss_qro_receta_detalle as d','d.id_receta','p.id_receta')
            ->leftJoin('cat_medico_ss_qro as m','m.id_medico','p.id_medico')
            ->leftJoin('cat_localidad as l', function($q) use($localidad){
                $q->on('l.id_localidad','=','p.id_localidad');
                $q->whereRaw("(l.id_localidad = $localidad or $localidad = -999)");
            })
        ...
        $char2 = DB::table('ss_qro_receta as p')
    	    ...
    	    ->selectRaw("d.clave_cliente, substring(c.descripcion from 1 for 200) as producto, sum(d.cantidad_pedida) as cantidad_pedida, sum(d.cantidad_pedida * c.precio)::numeric(10,2) as monto, concat('#',substring(md5(random()::text) from 4 for 6)) as color")
    	    ->where('l.id_cliente',135)
    	    ->where('l.tipo',0)
    	    ->where('l.estatus',1)
    	    ->whereNotNull('d.clave_cliente')
	    ...
	    $char3 = DB::table('ss_qro_receta as p')
    	    ->leftJoin('ss_qro_receta_detalle as d','d.id_receta','p.id_receta')
    	    ->leftJoin('cat_localidad as l', function($q) use($localidad){
    	        $q->on('l.id_localidad','=','p.id_localidad');
    	        $q->whereRaw("(l.id_localidad = $localidad or $localidad = -999)");
