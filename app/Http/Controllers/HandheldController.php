...
class HandheldController extends Controller
{
	...
	public function almacenes(Request $request,$company)
	{
		return view('handheld.almacenes', [
			...
			'sucursal' => $request,
		    'almacenes'  => Almacenes::activos()->where('fk_id_sucursal',$request->id)->get(),
		]);
		// dd($almacenes);
	}
	...
	public function movimientos(Request $request,$company)
	{
		return view('handheld.movimientos', [
			...
			'almacen' => $request->id,
		    'stock'  => Stock::activos()->where('fk_id_almacen',$request->id)->with('upc:id_upc,upc,nombre_comercial,descripcion','almacen:id_almacen')->select('id_stock','fk_id_sku','fk_id_upc','lote','fecha_caducidad','stock','fk_id_ubicacion')->get(),
		]);
	}
