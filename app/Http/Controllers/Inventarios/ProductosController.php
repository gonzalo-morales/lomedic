...
class ProductosController extends ControllerBase
{
    ...
    {
        $grupos = GrupoProductos::activos()->pluck('grupo','id_grupo')->sortBy('grupo');

        foreach ($grupos as $id => $grupo) {
            $subgrupo = SubgrupoProductos::where('fk_id_grupo',$id)->activos()->pluck('subgrupo','id_subgrupo')->sortBy('subgrupo')->toArray();
            if(!empty($subgrupo))
            { $subgrupos[$grupo] = $subgrupo; }
        }

        return [
            'seriesku' => SeriesSkus::activos()->pluck('nombre_serie','id_serie_sku')->sortBy('nombre_serie')->prepend('Selecciona una opcion...',''),
            'unidadmedida' => UnidadesMedidas::activos()->pluck('nombre','id_unidad_medida')->sortBy('nombre')->prepend('Selecciona una unidad de medida',''),
            'subgrupo' => collect($subgrupos ?? [])->prepend('Selecciona un subgrupo','')->toArray(),
            'impuesto' => Impuestos::activos()->pluck('impuesto','id_impuesto')->sortBy('impuesto')->prepend('Selecciona una opcion...',''),
            'familia' => FamiliasProductos::activos()->pluck('descripcion','id_familia')->sortBy('descripcion')->prepend('Selecciona una familia...',''),
            'presentacionventa' => PresentacionVenta::activos()->pluck('presentacion_venta','id_presentacion_venta')->sortBy('presentacion_venta')->prepend('Selecciona una Presentacion de venta...',''),
            'metodovaloracion' => MetodosValoracion::activos()->pluck('metodo_valoracion','id_metodo_valoracion')->sortBy('metodo_valoracion')->prepend('Selecciona una opcion...',''),
            'periodos' => Periodos::activos()->pluck('periodo','id_periodo')->sortBy('periodo')->prepend('Selecciona una opcion...',''),
            'sociosnegocio' => SociosNegocio::activos()->whereNotNull('fk_id_tipo_socio_compra')
                ->pluck('nombre_comercial','id_socio_negocio')->sortBy('nombre_comercial')->prepend('Selecciona un Proveedor...',''),
            'upcs' => Upcs::activos()->select('id_upc','upc')->pluck('upc','id_upc')->sortBy('upc')->prepend('Selecciona un upc',''),
    ...
    public function obtenerSkus($company,Request $request)
    {
        ...
        $skus = null;
        if($id_proyecto) {
            $skus = Productos::activos()->whereRaw("(sku ILIKE '%$term%' OR descripcion_corta ILIKE '%$term%' OR descripcion ILIKE '%$term%')")->
            ...
            $skus = Productos::activos()->whereRaw("(sku ILIKE '%$term%' OR descripcion_corta ILIKE '%$term%' OR descripcion ILIKE '%$term%')")->whereIn('id_sku',SociosNegocio::find($id_socio)->productos->pluck('fk_id_sku'))->get();
        }
        $skus_set = []; 
        foreach ($skus as $sku)
        {
