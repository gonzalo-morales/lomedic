...
class OfertasController extends ControllerBase
{
	...
	{
	    return [
	        'companies' => Empresas::activos()->where('conexion','<>',request()->company)->where('conexion','<>','corporativo')->activos()->pluck('nombre_comercial','id_empresa'),
	        ...
	        'monedas'=>Monedas::activos()->select('id_moneda',DB::raw("concat(descripcion,' (',moneda,')') as moneda"))->pluck('moneda','id_moneda'),
            'proyectos' => !empty($entity) ? Proyectos::where('fk_id_estatus',1)->pluck('proyecto','id_proyecto') : ['0'=>'Sin proyecto'],
	        'unidadesmedidas'=>UnidadesMedidas::activos()->pluck('nombre','id_unidad_medida'),
            ...
            "clientes"=>SociosNegocio::activos()->whereHas('empresas',function ($q){
                $q->where('conexion',\request()->company);
...
//        return view(currentRouteName('imprimir'));
    ...
    public function getProveedores($company){
        $proveedores = SociosNegocio::activos()->whereNotNull('fk_id_tipo_socio_compra')->select('id_socio_negocio as id','nombre_comercial as text','tiempo_entrega')->get();
