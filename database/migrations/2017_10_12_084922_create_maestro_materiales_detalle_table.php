...
class CreateMaestroMaterialesDetalleTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::connection('abisa')
            ->create('pry_det_proyectos_productos', function (Blueprint $table) {
                $table->increments('id_clave_cliente_proyecto');
    ...
    {
        Schema::connection('abisa')
            ->dropIfExists('pry_det_proyectos_productos');
        Schema::connection('lomedic')
            ->dropIfExists('pry_det_proyectos_productos');
    }
}
